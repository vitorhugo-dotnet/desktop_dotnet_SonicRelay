...
On non-PR runs (pushes to `main` and manual dispatches), the same workflow also publishes release assets for both platforms — Windows `SonicRelay.WindowsPublisher-win-x64-<version>.zip`/`.exe`/`.msi`, then Linux `SonicRelay-LinuxPublisher-linux-x64-<version>.tar.gz`/`.deb`/`.rpm`/`.AppImage`, plus one shared `checksums-sha256.txt` — uploaded back to the workflow run, and to a `dev-<run-number>` (or manually chosen) prerelease on GitHub Releases.
