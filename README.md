...
Alongside the direct-download assets, both workflows build an `x64` **MSIX** package for the Microsoft Store from the same commit, and upload it as the `store-package-<version>` workflow artifact rather than as a release asset — it is unsigned, because the Store re-signs every package it ingests, so only the Partner Center can do anything with it. See [`docs/microsoft-store-package.md`](docs/microsoft-store-package.md) for the package identity, the capabilities it declares (only `runFullTrust` — SonicRelay never opens a microphone), how to build and sign one locally, and the install/update/uninstall and Windows App Certification Kit checks that gate a submission.
...
See [the publisher specification](docs/windows-publisher.md), [the Linux publisher notes](docs/linux-publisher.md), [the macOS publisher notes](docs/macos-publisher.md), [the Microsoft Store package notes](docs/microsoft-store-package.md), [architecture notes](docs/architecture.md), [non-admin checklist](docs/non-admin-checklist.md), and [release smoke test](docs/release-smoke-test.md) for the planned system boundaries and release gates.
